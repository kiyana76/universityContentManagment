...
@section('content')
    <div class="card card-default">
        ...
        <form method="POST" role="form" action="{{ route('notes.update', $note->id) }}">
            @csrf
            @method('PUT')
            <div class="card-body">
                <div class="row">
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-sm-12">
                            <select class="form-control select2" name="field_id" required>
                                <option value="">انتخاب کنید</option>
                                @foreach($fields as $item)
                                    <option value="{{ $item->id }}" {{ $note->resource->fields->first()->id == $item->id ? 'selected' : '' }}>{{ $item->full_name }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
