...
@section('content')
    <div class="card card-default">
        ...
        <form method="POST" role="form" action="{{ route('groups.update', $group->id) }}">
            @csrf
            @method('PUT')
            <div class="card-body">
                <div class="row">
                    <div class="form-group col-sm-6">
                        ...
                        <input type="text" class="form-control" id="title" name="title" placeholder="عنوان" required value="{{ $group->title }}">
                    </div>
                    ...
                    <div class="form-group col-sm-6">
                        ...
                        <div class="form-check">
                            ...
                            <input id="enable" name="status" class="form-check-input" type="radio" value="enable" {{ $group->status == 'enable' ? 'checked' : '' }} required>
                            <label for="enable" class="form-check-label">فعال</label>
                        </div>
                        <div class="form-check">
                            <input id="disable" name="status" class="form-check-input" type="radio" value="disable" {{ $group->status == 'disable' ? 'checked' : '' }}>
